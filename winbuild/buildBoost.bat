...
b2 --prefix=%BUILD_DIR% --toolset=msvc-12.0 architecture=x86 address-model=64 --build-type=complete variant=release link=shared threading=multi -s ZLIB_INCLUDE=%BUILD_DIR%\include -s ZLIB_LIBPATH=%BUILD_DIR%\lib -s ZLIB_BINARY=zlib install
